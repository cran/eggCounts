...
set.seed(1)
t1 <- stan2mcmc(fec_stan(count[,"obs"], rawCounts=FALSE, CF=50, zeroInflation = TRUE, nburnin=10,nsamples=1e3,thinning=1)$stan.samples)
set.seed(1)
t2 <- stan2mcmc(fec_stan(count[,"obs"]/50, rawCounts=TRUE, CF=rep(50,10), zeroInflation = TRUE, nburnin=10,nsamples=1e3,thinning=1)$stan.samples)
...
#          nburnin=500,thinning=10,nchain=2,ncore=1,verbose=TRUE)$stan.samples
...
t6<-fecr_stan(counts[,"masterPre"],counts[,"masterPost"],rawCounts=TRUE,paired=TRUE,zeroInflation = TRUE,nsamples=1000,
              nburnin=500,verbose=TRUE)$stan.samples
